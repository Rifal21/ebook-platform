...
<div>
    @if ($showForm)
        <div
            class="reveal active bg-white dark:bg-slate-900 rounded-[40px] p-6 md:p-10 shadow-sm border border-slate-100 dark:border-white/5 mb-12">
            <div class="flex justify-between items-center mb-10">
                <h4 class="text-2xl font-black text-slate-900 dark:text-white">
                    {{ $editingEbookId ? 'Edit Produk' : 'Tambah Produk Baru' }}</h4>
            ...
            <form wire:submit.prevent="save" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            @if ($cover && !is_string($cover))
                                <div class="mb-4 rounded-2xl overflow-hidden border-2 border-slate-100 dark:border-white/10 relative shadow-sm">
                                    <img src="{{ $cover->temporaryUrl() }}" class="w-full h-32 object-cover">
                        ...
                        <div>
                            ...
                            @if ($file && !is_string($file))
                                <div class="mb-4 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 rounded-xl p-4 flex items-center space-x-3 shadow-sm">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-xs font-bold text-emerald-700 dark:text-emerald-400 truncate">{{ $file->getClientOriginalName() }}</p>
                                        <p class="text-[10px] text-emerald-600/70 dark:text-emerald-400/70 font-black uppercase">{{ round($file->getSize() / 1024, 1) }} KB</p>
                                    </div>
                                </div>
                            @endif
                            <input type="file" wire:model="file"
                                class="text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 w-full">
                            @error('file')
                                <span class="text-red-500 text-xs mt-2 block ml-2">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="md:col-span-2 flex justify-end space-x-4 mt-8">
                    ...
                    <button type="submit" class="btn-premium px-12 py-4">SIMPAN PRODUK</button>
                </div>
            </form>
        </div>
    @endif
    ...
    <div
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
            <div>
                <h4 class="text-2xl font-black text-slate-900 dark:text-white">Daftar Produk</h4>
                <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Total {{ $ebooks->count() }} koleksi aktif.
                </p>
            </div>
            <button wire:click="create" class="w-full md:w-auto btn-premium px-8 py-3 text-sm flex items-center justify-center space-x-2">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr
                        ...
                        <th class="py-6 px-4">Info Produk</th>
                        ...
                        <tr class="group hover:bg-slate-50 dark:hover:bg-white/5 transition-all">
                            ...
                            <td class="py-8 px-4 text-right pr-8">
                                <div class="flex justify-end space-x-2">
                                    ...
                                    <button onclick="confirm('Hapus produk ini?') || event.stopImmediatePropagation()"
